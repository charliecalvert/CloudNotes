...
const debugCall = createDebugMessages('lib:run-listDirs:call');
...
function findGuidePair(directory, guidePairs) {
    debugCall('findGuidePair called', directory, guidePairs);
    // iterate through guidePairs to find the one
    // where guidePair.path matches directory
    let guidePair = {};
    // iterate through guidePairs object properties
    // to find where guidePair.path matches directory
    ...
    /* for (const guidePair of guidePairs) {
        if (directory.includes(guidePair.path)) {
            return guidePair;
        }
    } */
    return null;
