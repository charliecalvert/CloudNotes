...
import { readFileAsync } from 'elven-code/elf-utils.js';

/* Debug Symbols */
import createDebugMessages from 'debug';
const debug = createDebugMessages('check-md');
const debugCheckGetElfCode = createDebugMessages('check-get-elf');
...
async function setupElfCode(fileName, relativePath, count) {
    const elfCodes = await getElfCode(fileName);
    elfCodes.id = count;
    return await addElfCode(fileName, relativePath, elfCodes);    
}
